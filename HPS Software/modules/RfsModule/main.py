...
from package.wildfiresensor import *
from package.wildfirethresholdcontroller import *
...
model_id ="dtmi:EM021:Wildfire:DE10_Nano;1" 
useComponent = True
...
wildfire = WildfireSensor(name="wilfire", real=False)
wildfirethc = WildfireThresholdController(real = False)
...
async def main():
    try:
        ...
        delay = 10
        watchdog_task  = None
        global isEdge, isReal, control_bridge, data_bridge, gs, rfs, thc, wildfire
        ...
        if(hostname in 'de10nano'):
            ...
            if ( bridges[0] is not None ) : 
                ...
                wildfire = WildfireSensor(name='wildfire', real=True, offset=0x40300)
                thc = ThresholdController(real=True,bridge=data_bridge,offset=0x40200)
                wildfirethc = WildfireThresholdController(real=true,bridge=data_bridge,offset=0x40400)
            ...
            while True:
                try :
                    if(isReal) :
                        ...
                        wildfire_data = wildfire.get_telemetries(data_bridge)
                    else : 
                    ...
                    logger.debug(f'Sent message: {msg}')
                    await client.send_message(msg)
                    msg = wildfire.create_component_telemetry(wildfire_data)
                    logger.debug(f'Sent message: {msg}')
                    await client.send_message(msg)
